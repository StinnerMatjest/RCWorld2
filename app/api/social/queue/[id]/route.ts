...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { caption, hashtags, image_url, scheduled_at, platforms, status: bodyStatus } = body;
  ...
  await pool.query(
    `UPDATE social_posts
     ...
     WHERE id = $7`,
    [caption ?? null, hashtags ?? null, image_url ?? null, scheduled_at ?? null, platforms ?? null, derivedStatus, id, hasScheduledAt]
  );
