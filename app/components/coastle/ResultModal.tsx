...
import { XMarkIcon, ShareIcon, ChartBarIcon } from "./Icons";
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      ...
      `}>
        <div className="bg-white dark:bg-neutral-900 rounded-[22px] p-6 text-center relative overflow-hidden flex flex-col">
          ...
          <button onClick={onClose} className="absolute top-3 right-3 p-2 bg-slate-100 dark:bg-neutral-800 rounded-full text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition z-10 cursor-pointer">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex gap-3">
              ...
              {gameMode === 'endless' && (
                <button onClick={onReset} className="flex-1 py-3.5 rounded-xl font-bold text-white shadow-lg bg-gradient-to-r from-blue-600 to-indigo-600 hover:brightness-110 cursor-pointer">
                  Play Again
                </button>
              )}
              <button 
                onClick={() => { onClose(); onShowStats(); }} 
                className={`${gameMode === 'endless' ? 'flex-none px-4' : 'flex-1 py-3.5'} rounded-xl font-bold text-slate-600 dark:text-slate-300 border-2 border-slate-100 dark:border-neutral-800 hover:bg-slate-50 dark:hover:bg-neutral-800 cursor-pointer`}
              >
