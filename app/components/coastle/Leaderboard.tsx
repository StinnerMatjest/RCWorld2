        ...
        <div className="grid grid-cols-4 gap-4 mb-10">
          {[
            ...
            <div key={s.label} className="flex flex-col items-center">
              <div className="text-3xl font-black text-slate-100">{s.val}</div>
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{s.label}</div>
            </div>
          ))}
        </div>

        <div className="text-left bg-neutral-800/50 p-6 rounded-2xl">
            ...
            {stats.guessDistribution.map((count, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-3 text-sm font-bold">
                  ...
                  <div
                    className={`h-8 flex items-center justify-end px-3 rounded-lg ${count > 0 ? 'bg-blue-500 text-white' : ' text-slate-400 bg-neutral-700'}`}
                    style={{ width: `${width}%` }}
                  >
                    {count}
                  </div>
