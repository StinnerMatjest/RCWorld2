    ...
    <div ref={ref} className="relative">
      ...
      <button
        ...
        className={`w-full flex items-center gap-2 px-2 py-2 rounded-xl text-left transition-colors ${
          hasMultiple ? "hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer" : "cursor-default"
        }`}
      >
        <p className="min-w-0 flex-1 text-sm md:text-xs font-medium text-gray-500 dark:text-gray-400 leading-tight truncate">
        ...
        {hasMultiple && (
          <svg
            className={`w-3.5 h-3.5 flex-shrink-0 text-gray-300 dark:text-gray-600 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
                ...
                <li key={r.id}>
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <p className={`text-xs font-medium leading-tight ${isCurrent ? "text-blue-600 dark:text-blue-400" : "text-gray-700 dark:text-gray-300"}`}>
                        {formatDate(r.date)}
                      </p>
                    </div>
                    <div className="flex items-center gap-0.5 flex-shrink-0">
                      <span className={`text-sm font-bold tabular-nums ${getRatingColor(r.overall)}`}>
                        {r.overall}
                      </span>
                      <span className="text-[11px] text-gray-400">/10</span>
