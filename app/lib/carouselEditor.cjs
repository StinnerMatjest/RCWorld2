...
(function(){
  ...
  const EDITABLE='.eyebrow, .h-title, .ho-name, .ride, .score, .ff-nums span, .ff-teams span, .ff-sub, .why, .next, .hk-name, .hk-sub, .hk-sub2, .sb-num, .progress-label, .inset .cap span, .cta-headline, .cta-question, .cta-line';
  ...
  // Scoring: a win is one point, a draw is one point each. The scoreboard shows the running total and
  // colours the side that won THIS round (both on a draw). The final slide takes the end total.
  function syncScores(){
    let aw=0,dlp=0;
    slides().forEach(slide=>{
      const isFinal=slide.classList.contains('final'),isHook=slide.classList.contains('hook');
      const hasFight=!!(slide.querySelector('.vs-top')&&slide.querySelector('.score-block'));
      let w=null;
      if(hasFight&&!isFinal&&!isHook){w=winnerOf(slide);if(w==='aw')aw++;else if(w==='dlp')dlp++;else if(w==='tie'){aw++;dlp++;}}
      const t=[...slide.querySelectorAll('.sb-team')];
      if(t.length===2){
        const a=isHook?0:aw,d=isHook?0:dlp;
        t[0].querySelector('.sb-num').textContent=a;t[1].querySelector('.sb-num').textContent=d;
        const fw=isFinal?(aw>dlp?'aw':dlp>aw?'dlp':'tie'):w;
        t[0].classList.toggle('lead',fw==='aw'||fw==='tie');t[1].classList.toggle('lead',fw==='dlp'||fw==='tie');
      }
      if(isFinal){const W=slide.querySelector('.ff-nums .w'),L=slide.querySelector('.ff-nums .l');if(W)W.textContent=aw;if(L)L.textContent=dlp;}
    });
  }
  ...
  window.__ed={slides,save,init,select,syncScores,renderCrop,cleanTrackHtml,ensureMeta,bindPhoto,bindText,SYNC};
